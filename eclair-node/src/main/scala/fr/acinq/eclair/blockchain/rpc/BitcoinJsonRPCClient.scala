...
class BitcoinJsonRPCClient(user: String, password: String, host: String = "127.0.0.1", port: Int = 1441, ssl: Boolean = false)(implicit system: ActorSystem) {
  ...
  def invoke(method: String, params: Any*)(implicit ec: ExecutionContext): Future[JValue] =
    for {
      ...
      jsonRpcRes <- Unmarshal(httpRes).to[JsonRPCResponse].map {
        ...
        case t: Throwable if httpRes.status == StatusCodes.Unauthorized => throw new RuntimeException("groestlcoind replied with 401/Unauthorized (bad user/password?)", t)
      }
