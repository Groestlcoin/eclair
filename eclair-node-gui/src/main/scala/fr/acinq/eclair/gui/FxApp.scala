...
class FxApp extends Application with Logging {
  ...
  def onError(t: Throwable): Unit = t match {
    ...
    case e@BitcoinRPCConnectionException =>
      notifyPreloader(new ErrorNotification("Setup", "Could not connect to Groestlcoin Core using JSON-RPC.", e))
    ...
    case e@BitcoinZMQConnectionTimeoutException =>
      notifyPreloader(new ErrorNotification("Setup", "Could not connect to Groestlcoin Core using ZMQ.", e))
