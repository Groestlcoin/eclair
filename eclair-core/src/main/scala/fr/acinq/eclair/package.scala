    ...
    Try(Base58Check.decode(address)) match {
      ...
      case Failure(base58error) =>
        Try(Bech32.decodeWitnessAddress(address)) match {
          ...
          case Success(("grs", _, bin)) if chainHash == Block.LivenetGenesisBlock.hash => OP_0 :: OP_PUSHDATA(bin) :: Nil
          case Success(("tgrs", _, bin)) if chainHash == Block.TestnetGenesisBlock.hash => OP_0 :: OP_PUSHDATA(bin) :: Nil
          case Success(("grsrt", _, bin)) if chainHash == Block.RegtestGenesisBlock.hash => OP_0 :: OP_PUSHDATA(bin) :: Nil
