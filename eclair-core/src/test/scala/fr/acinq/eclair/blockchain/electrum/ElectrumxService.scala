...
trait ElectrumxService extends DockerTestKit {
  ...
  val electrumxContainer = if (System.getProperty("os.name").startsWith("Linux")) {
    ...
    DockerContainer("acinq/electrumx")
      ...
      .withEnv(s"DAEMON_URL=http://foo:bar@localhost:$bitcoindRpcPort", "COIN=Groestlcoin", "NET=regtest", s"TCP_PORT=$electrumPort")
    ...
    DockerContainer("acinq/electrumx")
      ...
      .withEnv(s"DAEMON_URL=http://foo:bar@host.docker.internal:$bitcoindRpcPort", "COIN=Groestlcoin", "NET=regtest", s"TCP_PORT=$electrumPort")
      //.withLogLineReceiver(LogLineReceiver(true, println))
  }
